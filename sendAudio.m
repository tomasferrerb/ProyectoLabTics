...
function senal = sendAudio(Matrix)%recibe vector de bits
settings;
pixel_dt = 0:1/fs:info_pixel_f; % aca, en frecs
[row prof]=size(hiperMatrix);
...
for j=1:col
    px=[hiperMatrix(i,j,1) hiperMatrix(i,j,2) hiperMatrix(i,j,3) hiperMatrix(i,j,4)];% sacar el pixel como un vector de doubles RGB
    px = double(px);
    px_bi=[de2bi(px(1),8) de2bi(px(2),8) de2bi(px(3),8) de2bi(px(4),8)];
